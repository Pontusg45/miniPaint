  ...
  const API = {
    invScale: 0,
      ...
      im[0] = m[3] / cross;
		...
		toWorld(from_x: number, from_y: number) {  // convert screen to world coords
      const pointW = {
        x: 0,
        y: 0
      };
      if (dirty) {
		...
		toScreen(from: { x: number; y: number; }, point: { x: number, y:number} = {
      x: 0,
      y: 0
    ...
    scaleAt(x_from: number, y_from: number, amount: number) { // at in screen coords
			if (dirty) {
				this.update();
			}
			scale *= amount;
		...
		move(move_x: number, move_y: number) {  // move is in screen coords
			pos.x += move_x;
			pos.y += move_y;
			dirty = true;
		},
		...
		setBounds(top: number, left: number, right: number, bottom: number) {
			bounds.top = top;
			bounds.left = left;
			bounds.right = right;
