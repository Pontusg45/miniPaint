...
let databaseInitPromise: Promise<unknown> | null = null;
...
export default {
     ...
     */
    async get(imageId: string | number) {
        await this.init();
        if (database.isMemory) {
     ...
     */
    async delete(imageId: string | number) {
        await this.init();
        if (database.isMemory) {
