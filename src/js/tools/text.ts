...
function load_font_family({ family, variants }: { family: number; variants?: any; }, successCallback: { (): void; (): void; (): void; } | undefined) {
	if (fontLoadMap.get(family) == null) {
		...
		const loadPromise = new Promise((resolve, reject) => {
			WebFont.load({
				...
				fontinactive: (family) => {
					alert(`Font ${  family  } could not be loaded.`);
					fontLoadPromiseMap.delete(family);
					reject();
				}
			});
...
kerningTestCanvas.setAttribute("style", "font-kerning: normal; text-rendering: optimizeLegibility;");
const kerningTestCtx = kerningTestCanvas.getContext("2d");
class Font_metrics_class {
	family: string;
	size: number;
	kerningMap: Map<any, any>;
	width: number;
	height: number;
	baseline: number;
	 ...
	 */
	calculate_letter_bounds(letter: string, baseline: string) {
	 ...
	 */
	get_kerning_offset(letters: string | string[], flags = {}) {
...
class Text_document_class {
	...
	lines: any[] = [];
	on_change: any | null = null;
	queuedMetaChanges: any | null = null;
	 ...
	 */
	get_line_character_count(lineNumber: any) {
	 ...
	 */
	get_line_text(lineNumber: number) {
		let lineText = "";
	 ...
	 */
	get_word_end_position(line: number, character: number, noJump: boolean) {
		let newLine = line;
	 ...
	 * @param {boolean} noJump - Dont jump to the next word if at the end of current one
	 */
	get_word_start_position(line: number, character: number, noJump: boolean) {
		let newLine = line;
		let newCharacter = character;
		let isWrap = false;
	 ...
	 */
	is_same_span_meta(meta1: { [x: string]: any; }, meta2: { [x: string]: any; }) {
	 ...
	 */
	insert_empty_span(line: number, character: number, meta: { [x: string]: any; }) {
	 ...
	 * @param {number} character - The character position to insert at (0 indexed)
	 */
	insert_text(text: string | string[], line: number, character: number) {
	 ...
	 */
	delete_range(startLine: number, startCharacter: number, endLine: number, endCharacter: number) {
		// Check bounds
	 ...
	 */
	delete_character(forward: any, startLine: number, startCharacter: number) {
	 ...
	 * @param {number} endCharacter - The character position at the ending line of the text range
	 */
	get_meta_range(startLine: number, startCharacter: number, endLine: number, endCharacter: number) {
		// Check bounds
		startLine >= 0 || (startLine = 0);
		startCharacter >= 0 || (startCharacter = 0);
		endLine < this.lines.length || (endLine = this.lines.length - 1);
	 ...
	 */
	set_meta_range(startLine: number, startCharacter: number, endLine: number, endCharacter: number, meta: { [x: string]: any; }) {
		// Check bounds
		startLine >= 0 || (startLine = 0);
		startCharacter >= 0 || (startCharacter = 0);
		endLine < this.lines.length || (endLine = this.lines.length - 1);
	 ...
	 * @param {number} endLine - The ending line of the text range
	 */
	normalize(startLine: any, endLine: number) {
		for (let lineIndex = startLine; lineIndex <= endLine; lineIndex++) {
			const line = this.lines[lineIndex];
			let spanIndex = 0;
...
class Text_selection_class {
	...
	document: Text_document_class | null = null;

	constructor(/* Text_editor_class */ editor: Text_selection_class) {
		this.editor = editor;
	 ...
	 */
	compare_position(line1: number, character1: number, line2: number, character2: number) {
		if (line1 < line2) {
			return -1;
		} else if (line1 > line2) {
			return 1;
	 ...
	 */
	set_position(line: number | null, character: number, keepSelection = false) {
		if (line == null) {
			line = this.end.line;
		}
		if (character == null) {
		...
		// Check upper bounds
		const lineCount = this.editor.document?.get_line_count() || 0;
		line < lineCount || (line = lineCount - 1);
		const lineCharacterCount = this.editor.document?.get_line_character_count(line) || 0;
	 ...
	 * @param {boolean} isVisible 
	 */
	/* set_cursor_visible(isVisible: any) {
	...
	blinkIntervalHandle(blinkIntervalHandle: any) {
	 ...
	 */
	move_line_previous(length: number | null, keepSelection: any) {
		length = length == null ? 1 : length;
		const position = this.get_position();
		this.set_position(position.line - length, 0, keepSelection);
	}
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection
	 */
	move_line_next(length: null, keepSelection: any) {
		const newLength = length == null ? 1 : length;
		const position = this.get_position();
		this.set_position(position.line + newLength, 0, keepSelection);
	}
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_line_start(keepSelection: any) {
		const position = this.get_position();
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_line_end(keepSelection: any) {
		const position = this.get_position();
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_character_previous(length: number | null, keepSelection: any) {
		length = length == null ? 1 : length;
		const position = this.get_position();
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_character_next(length: null, keepSelection: any) {
		length = length == null ? 1 : length;
		const position = this.get_position();
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_word_previous(keepSelection: any) {
		const position = this.get_position();
	 ...
	 * @param {boolean} keepSelection - Whether to move to an empty selection or extend the current selection 
	 */
	move_word_next(keepSelection: any) {
		const position = this.get_position();
...
class Text_editor_class {
	...
	selectionTextColor: any;
	drawOffsetTop: any;
	drawOffsetLeft: any;
	shiftPressed: boolean;
	ctrlPressed: boolean;
	isMouseSelectionActive: boolean;
	mouseSelectionStartX: number;
	mouseSelectionStartY: number;
	mouseSelectionStartLine: null;
	mouseSelectionStartCharacter: null;
	mouseSelectionMoveX: null;
	mouseSelectionMoveY: null;
	mouseSelectionEdgeScrollInterval: null;
	focused: boolean;
	document: Text_document_class;
	wrappedLines: never[][];
	selection: Text_selection_class;
	layer: null;
	Base_layers: any;
	constructor(options?: { selectionBackgroundColor?: any; selectionTextColor?: any; paddingVertical?: any; paddingHorizontal?: any; } | undefined) {
		options = options || {};

		this.editingCtx = (document.getElementById("canvas_minipaint") as HTMLCanvasElement).getContext("2d");
	 ...
	 * @param {array} lines 
	 */
	set_lines(lines: { text: string; meta: {}; }[][]) {
	 ...
	 */
	get_wrap_text(wrap: { characterOffsets?: number[]; spans: any; }) {
		let wrapText = "";
	 ...
	 */
	get_span_font_metrics(span: { meta: { size: any; family: number; }; }, noCache: boolean) {
	...
	insert_text_at_current_position(text: any) {
	...
	delete_character_at_current_position(forward = false) {
		let newPosition;
		if (this.selection.is_empty()) {
			const position = this.selection.get_position();
	...
	trigger_cursor_start(layer: any, layerX: any, layerY: any) {
	...
	trigger_cursor_move(layer: any, layerX: any, layerY: any) {
	...
	get_cursor_position_from_absolute_position(layer: { params: { text_direction: any; wrap_direction: any; }; }, x: number, y: number) {
		let line = -1;
		let character = -1;
	...
	calculate_text_placement(ctx: CanvasRenderingContext2D, layer: Base_layers_class) {
	...
	render(ctx: CanvasRenderingContext2D, layer: { width: number; height: number; params: { boundary: number; text_direction: any; wrap_direction: any; }; y: number; x: number; rotate: number; }) {
								...
								if (!isSelectionEmpty) {
									isLetterSelected = (
										...
										(
											...
											(this.selection.start.line < lineIndex && this.selection.end.line > lineIndex)
									);
								}
...
// 	render_font_list(page: number | undefined) {
// 		page = page || 1;
...
// 		// Handle checkbox
// 		this.fontListNode.querySelectorAll("input[type=\"checkbox\"]").forEach((checkbox: { addEventListener: (arg0: string, arg1: (e: any) => void) => void; checked: any; value: string | number; }) => {
// 			checkbox.addEventListener("change", (e: any) => {
// 				if (checkbox.checked) {
// 					this.selectedFonts[checkbox.value] = this.fontListFiltered
// 						.slice(startIndex, startIndex + this.fontsPerPage)
// 						.filter((font: { family: number; }) => { return font.family === checkbox.value; })[0];
// 				} else {
...
// 		// Handle pagination
// 		this.fontListNode.querySelector(".pagination").addEventListener("click", (e: { target: { getAttribute: (arg0: string) => string; }; }) => {
...
// 			],
// 			on_load: (params: any, popup: { el: { querySelector: (arg0: string) => any; }; }) => {
// 				this.popup = popup;
...
// 				queryInput.addEventListener("input", (e: { target: { value: any; }; }) => {
...
// 				$.getJSON(`https://www.googleapis.com/webfonts/v1/webfonts?key=${apiKey}&sort=popularity`, (data: { items: any; }) => {
// 					this.fontList = data.items;
// 					this.fontListFiltered = data.items;
// 					this.render_font_list();
// 				}).fail(function () {
// 					alert("Error loading the list of fonts from Google.");
// 				});
// 			},
// 			on_finish: () => {
// 				this.popup = null;
...
class Text_class extends Base_tools_class {
	...
	constructor(ctx: CanvasRenderingContext2D) {
		...
		if (ctx) {
			...
			this.textarea.setAttribute("style", "position: absolute; top: 0; left: 0; padding: 0; width: 1px; height: 1px; background: transparent; border: none; outline: none; color: transparent; opacity: 0.01; pointer-events: none;");
			...
			this.textarea.addEventListener("input", (e) => {
				if (config.layer) {
					...
					editor.insert_text_at_current_position((e.target as HTMLInputElement).value);
				...
				if (config.layer) {
					...
					switch (e.key) {
						...
						case "b":
							if (e.ctrlKey) {
								e.preventDefault();
								(document.querySelector("#action_attributes #bold") as HTMLButtonElement).click();
								break;
							}
						case "c":
							if (e.ctrlKey) {
						...
						case "i":
							if (e.ctrlKey) {
								e.preventDefault();
								(document.querySelector("#action_attributes #italic") as HTMLButtonElement).click();
								break;
							}
						case "u":
							if (e.ctrlKey) {
								e.preventDefault();
								(document.querySelector("#action_attributes #underline") as HTMLButtonElement).click();
								break;
							}
						case "x":
							if (e.ctrlKey) {
	...
	mousedown(e: MouseEvent) {
		...
		if (existingLayer) {
			...
			const Insert_layer_action = new app.Actions.Insert_layer_action(layer);
			app.State?.do_action(
	...
	mousemove(e: MouseEvent) {
		...
		if (this.resizing && this.selection) {
			...
			this.get_editor(this.layer).trigger_cursor_move(this.layer, -1 + mouse.x - (this.layer.x ?? 0), mouse.y - (this.layer.y ?? 0));
		}
		this.Base_layers.render();
	}
	...
	update_tool_attributes(layer: { params: any; }, editor: { document: { get_meta_range: (arg0: any, arg1: any, arg2: any, arg3: any) => any; }; selection: { start: { line: any; character: any; }; end: { line: any; character: any; }; }; }) {
	...
	resize_to_dynamic_bounds(layer: { params: { boundary: string; }; }, editor: { textBoundaryWidth: number; textBoundaryHeight: number; }) {
	...
	render(ctx: CanvasRenderingContext2D, layer: Base_layers_class) {
		...
		if ((!this.resizing && isActiveLayerAndTextTool) && this.selection) {
			this.selection.x = layer.x;
			this.selection.y = layer.y;
			this.selection.width = layer.width;
			this.selection.height = layer.height;
			this.selection.rotate = layer.rotate || 0;
		} else if ((config.layer.type !== "text") && this.selection) {
	...
	get_text_layer_at_mouse(e: MouseEvent) {
