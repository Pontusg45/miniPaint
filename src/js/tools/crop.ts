...
class Crop_class extends Base_tools_class {
	...
	mousemove(e: MouseEvent) {
		...
		if(e.ctrlKey == true || e.metaKey){
			...
			let width_new = Math.round(height * ratio);
			let height_new = Math.round(width / ratio);
	...
	mouseup(e: MouseEvent) {
		...
		let width = mouse.x - this.selection.x;
		let height = mouse.y - this.selection.y;
	...
	async on_params_update() {
		...
		if (rotated_name !== false) {
			alert("Crop on rotated layer is not supported. Convert it to raster to continue." + `(${ rotated_name  })`);
			return;
		}
