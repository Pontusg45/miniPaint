...
class Brush_class extends Base_tools_class {
	...
	mousedown_action(e: MouseEvent, index: number, event_identifier: any) {
		...
		let current_group = config.layer.data[index];
		...
		let mouse_coords = this.get_mouse_coordinates_from_event(e);
	...
	mousemove_action(e: MouseEvent, index: number) {
		let mouse = this.get_mouse_info(e);
		if (mouse.is_drag == false)
			return;
		if (mouse.click_valid == false) {
	...
	mouseup_action(e: MouseEvent, index: undefined) {
		let mouse = this.get_mouse_info(e);
		/* if (mouse.click_valid == false) {
	...
	render(ctx: CanvasRenderingContext2D, layer: { data: string | any[]; params: any; color: any; x: number; y: number; }) {
		...
		let n = data.length;
		for (let k = 0; k < n; k++) {
			let group_data = data[k]; //data from mouse down till mouse release
			let group_n = group_data.length;
	 ...
	 * @param queue
	 */
	render_stabilized(ctx: CanvasRenderingContext2D, queue: any) {
		let data = JSON.parse(JSON.stringify(queue));
		let n = data.length;
		...
		let temp_data2 = [temp_data1[0]];
		for (let i = 1; i < temp_data1.length - 1;  i = i+1) {
			c = (temp_data1[i][0] + temp_data1[i + 1][0]) / 2;
			d = (temp_data1[i][1] + temp_data1[i + 1][1]) / 2;
			temp_data2.push([c, d]);
		}

		let temp_data = [temp_data2[0]];
		for (let i = 1; i < temp_data2.length - 1;  i = i+1) {
		...
		//draw
		for (let i = 1; i < temp_data.length - 2;  i = i+1) {
	...
	check_legacy_format(data: any[][][]) {
		//check for legacy format
		if(data.length > 0 && typeof data[0][0] == "number"){
			//convert
			let legacy = JSON.parse(JSON.stringify(data));
			data = [];
			data.push([]);
			let group_index = 0;
			for(let i in legacy){
