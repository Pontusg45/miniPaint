...
class Fill_class extends Base_tools_class {
	...
	async fill(mouse: MouseEvent | null) {
		...
		if (config.ALPHA == 0) {
			alert("Color alpha value can not be zero.");
			return;
		}
		...
		let color_to = this.Helper.hexToRgb(config.COLOR);
		...
		if (config.layer.type != null) {
			...
			params.x = canvas.dataset.x || 0;
			params.y = canvas.dataset.y || 0;
	...
	fill_general(context: CanvasRenderingContext2D, W: number, H: number, x: number, y: number, color_to: { r: number; g: number; b: number; a: number; }, sensitivity: number, anti_aliasing: string | number | boolean | object, contiguous = false) {
		...
		canvasTemp.width = W;
		canvasTemp.height = H;
		let ctxTemp = canvasTemp.getContext("2d") as CanvasRenderingContext2D; 
		...
		if (contiguous == false) {
			...
			while (stack.length > 0) {
				let curPoint = stack.pop() as number[];
				for (let i = 0; i < 4; i++) {
