...
class Cog_class extends Base_tools_class {
	...
	render_overlay(ctx: CanvasRenderingContext2D){
		// TODO const ctx = this.Base_layers.ctx;
	...
	render(ctx: CanvasRenderingContext2D, layer: { params: any; x: number; width: number; y: number; height: number; rotate: number; }) {
		...
		this.draw_shape(ctx, -layer.width / 2, -layer.height / 2, layer.width, layer.height, undefined);
