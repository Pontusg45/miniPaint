...
class Moon_class extends Base_tools_class {
	...
	render_overlay(ctx?: CanvasRenderingContext2D){
		...
		} else if (this.Base_layers.ctx) {
			this.render_overlay_parent(this.Base_layers.ctx);
		}
		else {
			throw new Error("No context to render overlay");
		}
	}
	...
	draw_shape(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, fill: boolean | number[][] | undefined, stroke: boolean | undefined) {
		let left = x;
		let top = y;
