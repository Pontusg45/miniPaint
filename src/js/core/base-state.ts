...
class Base_state_class {
	...
	Base_layers: Base_layers_class | null = null;
	Base_gui: Base_gui_class | null = null;
	Helper: Helper_class | null = null;
	layers_archive: Base_layers_class[] = [];
	levels: number = 3;
	levels_optimal: number = 3;
	enabled: boolean = false;
	action_history: Base_action[] = [];
	action_history_index: number = 0;
	action_history_max: number = 50;
	...
	async do_action(action: Base_action, options = {
    merge_with_history: ""
  }) {
	...
	async free(memory_size = 0, database_size = 0) {
		...
		while (this.action_history_index > 0) {
			...
			total_memory_freed += action?.memory_estimate ?? 0;
			total_database_freed += action?.database_estimate ?? 0;
			try {
				await action?.free();
			} catch (error) {
				has_error = true;
