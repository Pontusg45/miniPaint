...
class GUI_colors_class {
	...
	private colorNotSet: boolean;
	private uiType: null | string;
	private butons: {
		toggleColorSwatches: JQueryStatic | null;
		toggleColorPicker: JQueryStatic | null;
		toggleColorChannels: JQueryStatic | null;
	};
	private sections: {
		channelsPlaceholder: Comment;
		pickerPlaceholder: Comment;
		channels: null | JQueryStatic;
		picker: null | JQueryStatic;
		swatchesPlaceholder: Comment;
		swatches: JQueryStatic | null
	} | null;
	private inputs: {
		hsl: {
			s: {
		...
		pickerGradient: JQueryStatic | null;
		hex: string;
		rgb: {
			a: { number: JQuery<HTMLElement>; range: JQuery<HTMLElement>};
			r?: { number: JQuery<HTMLElement>; range: JQuery<HTMLElement> };
			b: { number: JQuery<HTMLElement>; range: JQuery<HTMLElement> };
			g: { number: JQuery<HTMLElement>; range: JQuery<HTMLElement> }
		};
		sample: JQueryStatic | null;
		swatches: JQueryStatic | null
	} | null;
	private readonly Helper: Helper_class;
	buttons: any;
	...
	constructor() {
		...
		this.uiType = null;
		this.butons = {
			toggleColorSwatches: null,
			toggleColorPicker: null,
			toggleColorChannels: null
		};
		this.sections = null;
		this.inputs = {
			sample: null,
	...
	render_main_colors(uiType?: string) {
	...
	init_components() {
		...
		this.inputs.sample.on("click", (event: any) => {
	 ...
	 */
	set_color(definition: { [x: string]: any; hex?: any; h?: any; s?: any; v?: any; r?: any; g?: any; b?: any; l?: any; a?: any; }) {
		let newColor = null;
		let newAlpha = null;
		let hsl = null;
		let hsv = null;
	 ...
	 */
	render_selected_color(options: { hsl?: any; hsv?: any; } | undefined) {
		options = options || {};
	 ...
	 *                    hsv - override for hsv values so it isn't calculated based on rgb (can lose selected hue/saturation otherwise)
	 */
	render_ui_deferred(options: { hsl?: any; hsv?: any; }) {
		options = options || {};
		const COLOR = this.uiType === "dialog" ? this.COLOR : config.COLOR;
		...
		for (const rgbKey in rgb) {
			const rangeMin = JSON.parse(JSON.stringify(rgb)) as {
				r: number;
				g: number;
				b: number;
			};
			const rangeMax = JSON.parse(JSON.stringify(rgb)) as {
				r: number;
				g: number;
				b: number;
			};
