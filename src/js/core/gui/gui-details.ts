...
class GUI_details_class {
	...
	render_main_details() {
		(document.getElementById("toggle_details") as HTMLElement).innerHTML = template;
	...
	render_details(events = false) {
		...
		if (config.layer != undefined && config.layer.type == "text") {
			(document.getElementById("text_detail_params") as HTMLElement).style.display = "block";
			(document.getElementById("detail_color")?.closest(".row") as HTMLElement).style.display = "none";
		}
		else{
			(document.getElementById("text_detail_params") as HTMLElement).style.display = "none";
			...
			if (config.layer != undefined && (config.layer.color === null || config.layer.type == "image")) {
				...
				//show color
				(document.getElementById("detail_color")?.closest(".row") as HTMLElement).style.display = "block";
			}
		}
	...
	render_general(key: string, events: boolean) {
		...
		if (layer != undefined) {
			const target = document.getElementById(`detail_${key}`);
			if(!target){
				console.log("Error: missing details target " + `detail_${key}`);
				return;
			}
			target.dataset.layer = layer.id.toString();
			if (layer[key] == null) {
				target.value = "";
		...
		if (events) {
			...
			let focus_value: number | null = null;
			...
			target.addEventListener("blur", function (e) {
				if(key == "x" || key == "y" || key == "width" || key == "height"){
					//convert units
					const value = _this.Helper.get_internal_unit(this.value, units, resolution);
				}
				else {
					const value = parseInt(this.value);
				}
				const layer = _this.Base_layers.get_layer(e.target.dataset.layer);
	...
	render_general_param(key: number, events: any) {
		const layer = config.layer;

		if (layer != undefined) {
			const target = document.getElementById(`detail_param_${  key}`);
	...
	render_general_select_param(key: string, events: boolean){
		const layer = config.layer;
	 ...
	 * item: color
	 */
	render_color(events: boolean) {
		const layer = config.layer;

		let $colorInput: { uiColorInput: (arg0: string, arg1: null | undefined) => void; on: (arg0: string, arg1: { (e: any): void; (e: any): void; }) => void; };
		if (events) {
		...
		if (events) {
			//events
			let focus_value: null = null;
			$colorInput.on("focus", function (e: any) {
				focus_value = $colorInput.uiColorInput("get_value");
			});
			$colorInput.on("change", function (e: any) {
	...
	render_more_parameters() {
		...
		const itemContainer = document.getElementById(target_id) as HTMLDivElement;
		...
		for (const k in params_config.attributes) {
			const item = params_config.attributes[k];
